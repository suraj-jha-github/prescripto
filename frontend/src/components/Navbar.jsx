      ...
      <div className='flex items-center gap-4 '>
        ...
        <div className={`md:hidden ${showMenu ? 'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
            ...
            {token && userData && (
              <>
                <button onClick={() => { navigate('/my-profile'); setShowMenu(false) }} className='px-4 py-2 rounded-full bg-gray-100'>My Profile</button>
                <button onClick={() => { navigate('/my-appointments'); setShowMenu(false) }} className='px-4 py-2 rounded-full bg-gray-100'>My Appointments</button>
                <button onClick={() => { logout(); setShowMenu(false) }} className='px-4 py-2 rounded-full bg-red-500 text-white'>Logout</button>
              </>
            )}
          </ul>
        </div>
